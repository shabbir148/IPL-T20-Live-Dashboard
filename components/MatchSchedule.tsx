      ...
      <div className="divide-y divide-gray-200">
        {matches.map((match) => (
          <div key={match.id} className="p-4 hover:bg-gray-50 transition-colors">
              ...
              <div className="flex items-center space-x-4">
                {match.status === 'live' && match.currentScore && (
                  <div className="text-sm text-gray-600">
                    <div>{match.team1.shortName}: {match.currentScore.team1Score}</div>
                    <div>{match.team2.shortName}: {match.currentScore.team2Score}</div>
                  </div>
                )}
            ...
            {/* Venue for desktop */}
            <div className="hidden md:block text-xs text-gray-500 mt-2 pl-44">
              {match.venue}
            </div>
          </div>
        ))}
